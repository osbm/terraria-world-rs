...
fn carve_maze(
    ...
    if depth > 5000 {  // Increased depth limit for better coverage
        return;
    }
    ...
    for &(dx, dy) in &dirs {
        ...
        if nx < 0
            || ny < 0
            || nx as usize >= visited.len()
            || ny as usize >= visited[0].len()
        {
            continue;
        }
        ...
        let passage_start_x = if dx > 0 { base_x + hall_size } else if dx < 0 { base_x - wall_size } else { base_x };
        let passage_start_y = if dy > 0 { base_y + hall_size } else if dy < 0 { base_y - wall_size } else { base_y };
    ...
    carve_maze(&mut world, start_x, start_y, &mut visited, 0, margin, &mut blocks_carved);
    ...
    println!("Visited {} out of {} total grid cells ({:.1}%)",
             visited_count, grid_width * grid_height,
