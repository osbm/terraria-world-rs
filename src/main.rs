...
fn main() {
    ...
    let mut world = World::from_file(&format!("{}/{}", test_worlds_dir, world_name)).expect("Failed to read world file");
    ...
    println!("Writing world to file...");
    world.save_as_wld(&format!("{}/{}", test_worlds_dir, "output.wld"))
        .expect("Failed to write world file");
    println!("World written successfully.");
